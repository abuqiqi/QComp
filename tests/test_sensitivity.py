"""验证通用结果转换为完整敏感度记录与 Markdown 的一致性。

主要内容：
- 独立 JSON 结果字段与不共享可变指标。
...
def test_case_record_serializable_and_independent():
    ...
    result = SimpleNamespace(
        ...
        plan=CompressionPlan(
            (CompressionTarget("block", "mpo", MPOSpec((2,), (2,), (1, 1))),)
        ),
        evaluations={
        ...
        layer_compressions={"block": CompressionMetrics(4, 2, 2.0, 16, 8, 2.0, 0.1)},
        model_compression=ModelCompressionMetrics(6, 4, 1.5, 24, 16, 1.5, 1),
    ...
    assert json.loads(json.dumps(record)) == {
            ...
            "targets": [
                {
                    "module_path": "block",
                    "representation": "mpo",
                    "spec": {"out_modes": [2], "in_modes": [2], "ranks": [1, 1]},
                }
            ]
        },
        ...
        "layers": {"block": asdict(result.layer_compressions["block"])},
        "model_ratio": 1.5,
        "compression_seconds": 2.5,
